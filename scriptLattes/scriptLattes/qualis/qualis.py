...
class Qualis:
    ...
    def analisarPublicacoes(self, membro, grupo):
		...
		if (not (qualis.encode('utf-8') == 'Qualis nao identificado')):
                    try:
                        arquivo = open("./trabalho/qualis.txt", "a+")
                    except:
                        arquivo = open("./trabalho/qualis.txt", "w+")
                    trocavalor = {'A1':'100','A2':'85','B1':'70','B2':'50','B3':'20','B4':'10','B5':'5','C':'0'}
                    breno_e_tuzin = trocavalor.get(qualis.encode('utf-8'),"0")
                    arquivo.write(breno_e_tuzin.encode('utf-8'))
                    arquivo.write('\n')
                pub.qualis = qualis
